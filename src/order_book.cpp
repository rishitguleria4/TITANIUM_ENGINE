...
void OrderBook::match_order(const Order &incoming) {
  ...
  if (incoming.side == Side::Buy) {
    for (auto ask_it = asks.begin();
         ask_it != asks.end() && ask_it->first <= incoming.price;) {
      ...
      while (!ask_orders.empty() && remaining_qty > 0) {
        ...
        TradeEvent trade{timestamp, incoming.order_id, resting.order_id,
    ...
    for (auto bid_it = bids.begin();
         bid_it != bids.end() && bid_it->first >= incoming.price;) {
      ...
      while (!bid_orders.empty() && remaining_qty > 0) {
        ...
        TradeEvent trade{timestamp, resting.order_id, incoming.order_id,
                         resting.price, traded_qty};
...
bool OrderBook::modify_order(uint64_t order_id, uint32_t new_price,
  ...
  std::cout << "MODIFIED: Order ID " << order_id << " -> Price: " << new_price
